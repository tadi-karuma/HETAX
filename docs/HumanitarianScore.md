...
Humanitarian Score は、**加点最大スコア合計が100点になるように構成自由な柔軟設計**を採用します。企業は業種や方針に応じて評価項目を組み合わせ、自己の社会的・倫理的行動をアピール可能です（ただし虚偽項目には減点あり）。

スコアは、以下の【必須項目】（除外不可）＋【自由加点項目】（組合せ選択制）で構成され、最大スコア合計が100点を上限とする。すべてのスコア項目は、既存の会計・労務データに基づいて自動計算が可能である。
